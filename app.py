...
def getUsersFriends(uid):
	cursor = conn.cursor()
	cursor.execute("SELECT frnd_id FROM friends WHERE user_id = '{0}'".format(uid))
	frnd_id_tuples = cursor.fetchall() #NOTE return a list of tuples, [(imgdata, pid, caption), ...]
	frnd_id_list = []
	for frnd_id_tuple in frnd_id_tuples:
		frnd_id = frnd_id_tuple[0]
		frnd_id_list.append(frnd_id)
	return frnd_id_list
...
def getUserEmails(uid_list):
	...
	for uid in uid_list:
		...
		user_email_tuple = cursor.fetchall()
		user_email = user_email_tuple[0][0]
